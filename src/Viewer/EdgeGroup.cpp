...
EdgeGroup::EdgeGroup( QMap<QString, osg::ref_ptr<Data::Edge> >* edges )
{
	this->edges = edges;
...
void EdgeGroup::initEdges()
{
    ...
    QMap<QString, osg::ref_ptr<Data::Edge> >::iterator i = edges->begin();
...
void EdgeGroup::synchronizeEdges()
{
	for ( unsigned int i = 0; i < edgeGroup->getNumChildren(); i++ ) {
        if ( edges->key( static_cast<Data::Edge*>( edgeGroup->getChild( i )->asSwitch() ), QString::number( -1 ) ) == QString::number( -1 ) ) {
			edgeGroup->removeChild( i );
		}
	}

    QMap<QString, osg::ref_ptr<Data::Edge> >::iterator ie = edges->begin();
